...
def make_model(data):
    ...
    for capacity in capacity_category:
        ...
        # Make GLM
        life_exp = min(result.params[0] / abs(result.params[1]),20)
        simulated_x = np.linspace(0,life_exp,10)
        predictions = result.predict()
        sm_pred = result.get_prediction(sm.add_constant(simulated_x))\
                  .summary_frame(alpha=0.1)
        print(sm_pred.head(),flush=True)
